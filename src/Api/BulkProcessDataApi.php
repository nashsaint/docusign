...
namespace NashSaint\DocuSign\Api\BulkProcessDataApi;
...
class BulkProcessDataApi
{
    ...
    */
     ...
     * @param ?string $account_id The external account number (int) or account ID Guid.
     * @param ?string $process_action
     * @param \DocuSign\Model\BulkProcessingLists $bulk_processing_lists  (optional)
     *
     * @throws ApiException on non-2xx response
     * @return \DocuSign\Model\BulkProcessResult
     */
     ...
     *
     * @throws ApiException on non-2xx response
     * @return \DocuSign\Model\BulkProcessingListSummaries
     */
     ...
     *
     * @throws ApiException on non-2xx response
     * @return array of \DocuSign\Model\BulkProcessingListSummaries, HTTP status code, HTTP response headers (array of strings)
     */
    public function getBulkProcessListsWithHttpInfo($account_id, $process_action): array
    {
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\DocuSign\Model\BulkProcessingListSummaries',
                '/v2.1/accounts/{accountId}/bulk_process_data/actions/{processAction}'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\DocuSign\Model\BulkProcessingListSummaries', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\DocuSign\Model\BulkProcessingListSummaries', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
                case 400:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\DocuSign\Model\ErrorDetails', $e->getResponseHeaders());
